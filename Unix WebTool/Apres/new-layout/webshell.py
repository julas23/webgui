		...
		if pid == 0:
			if self.cmd:
				...
				if re.match('^[0-9A-Za-z-_.]+$', login):
					...
					cmd += ' -oNoHostAuthenticationFor192.168.0.254=yes'
					cmd += ' -oLogLevel=FATAL'
					cmd += ' -F/dev/null -l' + login +' 192.168.0.254'
				else:
					os._exit(0)
...
def main():
	...
	parser.add_option("-c", dest = "cmd", default = None, 
		help = "set shell command (default: ssh 192.168.0.254)")
	...
	parser.add_option("--ssl-disable", action = "store_false", dest = "ssl_enabled", default = 1,
		help = "disable SSL, set listen interface to 192.168.0.254")
