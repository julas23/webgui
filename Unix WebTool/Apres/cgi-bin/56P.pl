...
my %servers = ("tvmk8208" => " - 56P-ORT Search - App Search 1",
	           "tvmk8209" => " - 56P-ORT Search - App Search 2/3",
	           "tvmk8125" => " - 56P-ORT Search - App Scheduler",
     	       "tvmk8126" => " - 56P-ORT Search - App Staging",
);
...
if ($cgi->param("command")) {
	...
	foreach my $server ($cgi->param("servers")) {
		...
		$content .= "<div><h3>Server: <b>$server</b></h3>" . $cgi->checkbox("servers", 20, $server, "run again") . 
		      '&nbsp;' . $cgi->submit('Go') . "<br/>\n";
		$content .= "<font color=\"#FFFFF\"><pre class=\"unix_results\" style=\"background-color:black;\">";
	...
	$content .= $cgi->end_form;
		...
		$content = "56P-ORT Search Web Servers <br>";
